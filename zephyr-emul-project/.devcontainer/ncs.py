...
TOOLCHAINS_JSON = Path("/root/ncs/toolchains/toolchains.json")
...
def install_version(target_ver: str):
    sdk_dir = Path(f"/root/ncs/{target_ver}")
    ...
    if not has_tc:
        ...
        tc_cmd = f"nrfutil toolchain-manager install --ncs-version {target_ver}"
    ...
    if not zephyr_dir.exists():
        ...
        west_cmd = (
            f"nrfutil toolchain-manager launch --ncs-version {target_ver} -- "
            f"bash -c 'cd {sdk_dir} && west init -m https://github.com/nrfconnect/sdk-nrf --mr {target_ver} . && west update --narrow -o=--depth=1'"
        )
    ...
    subprocess.run("rm -rf /root/ncs/downloads/*", shell=True)
...
def list_versions(toolchains):
    ...
    if not versions:
        print('echo "No installed nRF Connect SDK versions found in /root/ncs." >&2')
...
def clean_path_var(var_name: str) -> str:
    ...
    return ":".join([e for e in entries if not e.startswith("/root/ncs/")])
...
def main():
    ...
    tc_path = Path(f"/root/ncs/toolchains/{hash_dir}")
    zephyr_base = Path(f"/root/ncs/{target_ver}/zephyr")
